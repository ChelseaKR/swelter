...
PARAMETERS: Final[dict[str, Parameter]] = {
    ...
    "heat_index_c": Parameter("heat_index_c", "degC", -40.0, 80.0),
}
...
class Observation:
    ...
    def content_hash(self) -> str:
        """Stable SHA-256 over the value-bearing fields, for integrity and dedup."""
        payload = json.dumps(
            [
                self.node_id,
                self.timestamp,
...
def pm25_aqi(concentration: float) -> tuple[int, str]:
    ...
    Uses the EPA piecewise-linear formula. Concentrations above the top breakpoint are
    clamped to 500/"Hazardous"; negatives are clamped to 0/"Good".
    """
    if concentration <= 0:
        return 0, "Good"
