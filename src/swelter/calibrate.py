...
def fit_one(node_id: str, parameter: str, pairs: list[TrainingPair], reference: str) -> Correction:
    ...
    coefficients = tuple(round(b, PRECISION) for b in beta[:-1])
    intercept = round(beta[-1], PRECISION)
    ...
    residual_std = round((ss_res / n) ** 0.5, PRECISION)
    r2 = round(1 - ss_res / ss_tot, PRECISION)
...
def humidity_index(observations: Iterable[Observation]) -> dict[tuple[str, str], float]:
    """Map (node_id, timestamp) → humidity, so a PM correction can find its co-time humidity."""
    return {
        (o.node_id, o.timestamp): o.value
        for o in observations
        if o.parameter == "humidity_pct" and o.calibration == RAW
    }
