...
def explode(payload: dict[str, Any]) -> tuple[list[Observation], str | None]:
    ...
    for name, param in PARAMETERS.items():
        ...
        if not math.isfinite(numeric):
            # NaN / Infinity is not a measurement: a NaN would be coerced to SQL NULL and
            # silently dropped, and an Infinity would poison the JSON export. Skip it.
            continue
        observations.append(
            Observation(
                node_id=node,
...
def ingest(
    ...
    *,
    ...
    for payload in payloads:
        parse_error = payload.get("__parse_error__")
        if parse_error:
            quarantine.append({"reason": parse_error, "payload": payload.get("__raw__")})
...
def read_jsonl(path: str | Path) -> Iterator[dict[str, Any]]:
    ...
    A line that is not valid JSON (or not a JSON object) yields a quarantine marker instead of
    raising, so a single corrupt line in a store-and-forward buffer is quarantined rather than
    poisoning the whole batch and losing every valid reading alongside it.
    """
    with Path(path).open(encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            try:
                ...
                yield {"__parse_error__": "unparseable JSON line", "__raw__": line}
                continue
            if isinstance(obj, dict):
                yield obj
            else:
                yield {"__parse_error__": "JSON line is not an object", "__raw__": line}
