...
class _Collector(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "html":
            ...
            page.control_total += 1
            if a.get("id"):
                page.control_ids.add(a["id"])
            if a.get("aria-label") or a.get("aria-labelledby"):
                page.controls_with_aria += 1
        elif tag == "img":
            page.imgs_total += 1
            if "alt" in a:
...
def _checks(page: Page, css: str) -> list[tuple[bool, str]]:
    unlabelled = (
        page.control_total - len(page.control_ids & page.label_for) - page.controls_with_aria
    )
    return [
