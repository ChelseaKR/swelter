...
DEFAULT_N_NODES = 150
N_NODES = int(os.environ.get("SWELTER_DEMO_NODES", str(DEFAULT_N_NODES)))
...
def wbgt_c(temp_c: float, humidity_pct: float) -> float:
    ...
    return float(round(0.7 * tw + 0.3 * temp_c, 2))
...
def write_network_yaml(nodes: list[Node], *, root: Path = ROOT) -> None:
    doc = {
        "name": "swelter demo network (downtown)",
        "grid_resolution_m": 150,
        # Only the canonical generated fixture passes config.py's exact fingerprint. Custom demo
        # sizes stay downtown and remain valid inputs instead of claiming a statewide preview.
        **({"web_preview": "statewide-california"} if len(nodes) == DEFAULT_N_NODES else {}),
        "languages": ["en", "es"],
        "reference_monitors": [
            {
    ...
    preview_note = (
        "# web_preview belongs only to this synthetic fixture; remove it from a real network.\n"
        if len(nodes) == DEFAULT_N_NODES
        else ""
    )
