...
```console
$ swelter export --since 2026-06-01 --format csv --bbox "downtown" > heat.csv
swelter: 41,902 observations from 18 nodes (12 calibrated, 6 raw-flagged)
         calibration applied: PM2.5 US-EPA correction v3; temp enclosure-offset v2
         coverage: 2026-06-01 → 2026-06-17, median gap 5 min, longest gap 41 min (node-07 offline)
...
- **Phase 4 — generalize.** A `network.yaml` so any community can register its nodes and reference
  monitors and stand up its own instance, with an "add your neighborhood in an afternoon" guide and the
  hardware build doc finished.
