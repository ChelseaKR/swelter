...
1. **Tree canopy** (reused, not re-sourced — `context_layers.py`, ADR 0023 already ships this).
...
new context-layer datasets that follow the pattern ADR 0023 set for tree canopy.
...
- **`swelter.ac_access_layer`** and **`swelter.redlining_layer`** are new sibling modules to
  ...
  (`source`, `source_url`, `last_verified`), the same discipline ADR 0023 established so no
...
- The committed datasets are **illustrative samples**, not real LACE/HOLC data, because the demo
  ...
  feature is safe to show to residents, exactly as ADR 0023 already requires for canopy.
...
- No map/dashboard wiring or translated (es) copy ships in this PR — `swelter brief` is a
  CLI/library surface today, the same staged scope ADR 0023 chose for canopy (loader/validator/CLI
