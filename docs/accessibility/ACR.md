...
- **Automated, merge-blocking:** the structural WCAG 2.2 AA gate `scripts/a11y_check.py`
  ...
  fails if any regresses. It holds the structural floor — a language, a single `<h1>`, landmarks,
  a working skip link, labelled controls, a real data-table equivalent to the map, image text
  alternatives, no positive `tabindex`, a language switch, a `prefers-reduced-motion` rule, and a
  visible focus indicator. As of this report all twelve pass.
