...
| Notes | The shell is plain HTML/CSS/ES modules plus complete 400-key English and Spanish MF2 catalogs. An install step pins the stable `messageformat@4.0.0` release and generates the local MF2 runtime; there is no bundler, runtime framework, map-tile request, external package request, or external font request. |
