...
2026-07-31. The MF2, JavaScript, cross-browser, Axe, Pa11y, and Lighthouse suites pass in CI and
locally. The expanded linked visualizations retain text equivalents, but NVDA on Windows, VoiceOver
on macOS/iOS, 200% zoom, and reflow must be re-run for the next formal conformance signoff. Recheck
cadence: regenerated and re-committed each release, and at least every 6 months when no release ships.
...
| Product description | A framework-free exposure observatory for neighborhood heat and air quality. Current reading leads into linked history, distribution, map, sortable table, plain list, and evidence-inspector views of one aggregated surface. Served by `swelter serve`; installable as a PWA; deployable to static hosting. |
| Contact information | Chelsea Kelly-Reif — github.com/ChelseaKR/swelter |
| Notes | The shell is plain HTML/CSS/ES modules plus complete 400-key English and Spanish MF2 catalogs. An install step pins `messageformat@4.0.0-11` and generates the local MF2 runtime; there is no bundler, runtime framework, map-tile request, external package request, or external font request. Stable `messageformat@4.0.0` has not been verified. |
| Evaluation methods used | See below. |

### Evaluation methods used

- **Automated, merge-blocking in CI:** `npm --prefix web run verify` runs MF2/unit contracts,
  Playwright plus Axe, Pa11y, and Lighthouse against distinct root and Sensor.Community route
  fixtures in Chromium, Firefox, and WebKit. Confirmed moderate-or-worse Axe findings and
  ...
  target-size check permits only the WCAG 2.5.8 inline-text and 24 CSS-pixel-spacing exceptions. On
  2026-07-31, the MF2 and JavaScript unit suite passed 117/117 checks; Playwright passed 57/57 checks
  across the three engines; Pa11y passed both routes; and Lighthouse passed both route budgets,
...
- **Manual screen-reader review:** the 2026-06-16 baseline covered NVDA on Windows (Firefox and
  ...
  control. The current observatory keeps those tested controls/IDs and adds text summaries for its
  decorative SVG, but its expanded linked-view sequence awaits the next full walkthrough with
  NVDA on Windows and VoiceOver on macOS/iOS; this limitation is stated rather than silently rolling
  the old date forward.
- **Keyboard browser review:** Playwright completes the primary path using only Tab /
  Shift+Tab, Enter / Space, Arrow, Home, End, and the documented single-key shortcut, while checking
  focus visibility and linked-view state. The current assertions pass in CI and locally; the
  2026-06-16 baseline remains the last human keyboard pass, so manual confirmation is still pending.
- **Text expansion and reflow:** Playwright switches through Map, List, and Table at a
  320 CSS-pixel viewport and under a 40%-expanded pseudolocale, checking page-level overflow and
  clipped marked copy in each view. Those automated checks pass. The 2026-06-16 baseline supplied
  the last manual 200% browser-zoom pass; manual zoom/reflow must be repeated with the expanded
  sequence.
- **Reduced motion:** Playwright emulates the operating-system reduced-motion
  preference and asserts authored animation, transition, and smooth scrolling are suppressed. No
  essential information is designed to be motion-only; the automated check passes.
...
| 1.3.2 Meaningful Sequence | Supports | DOM order matches reading order: Current reading, Readings heading and controls, linked visual summaries, representation tabs and panels, inspector, Network, then Data & Method. Responsive CSS changes columns, not semantic order. |
...
| 2.4.3 Focus Order | Supports | Focus order follows the visual and DOM order; the active tab is the single tab stop (roving `tabindex`), panel content follows it, a cluster activation returns focus to the named map group before the newly revealed markers, and rerendered map/list/table, ranking, overview, and alert controls restore focus to the equivalent control or the selected List row. |
...
| 2.5.1 Pointer Gestures | Supports | The map accepts pan/pinch, but none is required: cluster/zoom/reset buttons, arrow keys, selectable markers, List, and Table expose the same outcomes without a path-based or multipoint gesture. |
...
| 4.1.2 Name, Role, Value | Supports | Native controls expose name/role/value to the platform; map cluster buttons name their reading count and value range and expose collapsed/revealed state with `aria-expanded`; the tablist exposes `aria-selected`, selectable Map/List/Table and distribution buttons expose the linked location with `aria-pressed`, sortable headers expose `aria-sort` updated on sort, and the time readout uses `<output for>` with `aria-live="polite"`. |
...
| 1.4.3 Contrast (Minimum) | Partially Supports | Body text, labels, and control text are designed for at least 4.5:1, and large text and UI borders for 3:1, in both light and dark palettes. Category swatches pair color with patterns. Axe, Pa11y, and the browser calculation for patterned map-reading and braid-label pairs pass in CI and locally. The level remains partial pending the current manual zoom and assistive-technology signoff. |
...
| 1.4.5 Images of Text | Supports | The dashboard does not render images of text; all interface and data text is real text. The PWA icon and social-preview card are branding metadata, not page content or task instructions. |
| 1.4.10 Reflow | Partially Supports | The single-column responsive layout targets a 320 CSS-pixel width (`@media (max-width: 40rem)` and flex wrapping), and Playwright switches through Map, List, and Table while checking page-level overflow and clipped marked copy at that width with expanded text. The automated run passes; the manual zoom/reflow walkthrough remains pending. |
| 1.4.11 Non-text Contrast | Partially Supports | Control borders, the focus outline (`--focus`), cell borders, and tab boundaries are designed for 3:1 against adjacent surfaces in both palettes, and the browser contrast gate passes. Manual low-vision and zoom signoff remains pending. |
...
| 2.4.11 Focus Not Obscured (Minimum) | Partially Supports | Sticky command-bar and desktop-inspector layouts reserve scroll padding/margins; the inspector stops being sticky in the stacked layout. The passing Playwright assertion samples `elementsFromPoint` around every visible keyboard target across Map, List, and Table to reject full obscuration. Manual confirmation remains pending. (WCAG 2.2.) |
...
| 302.7 With Limited Manipulation | Partially Supports | Native and custom keyboard paths avoid required dragging, multipoint gestures, or timed actions. CI and local runs pass the keyboard path and exhaustive rendered-target geometry checks using only the inline-text and 24px-spacing exceptions; manual switch/touch confirmation remains pending. |
...
| 602.3 / WCAG conformance of web content | Partially Supports | Structural and browser gates pass for the criteria detailed above, but the expanded NVDA/VoiceOver/iOS/zoom/reflow walkthrough remains pending. Full conformance is not claimed until that manual review completes and its findings are resolved. |
