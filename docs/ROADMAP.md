...
verify` passes (fmt-check, lint, typecheck, a11y, test) with 62 tests. The worked example in
...
- `src/swelter/store.py` — `drop_calibrated()` rebuilds derived records from immutable raw.
...
- `docs/` — an "add your neighborhood in an afternoon" guide and the finished hardware build doc
...
| Type safety | `mypy --strict` passes | `mypy --strict` over `src/swelter/` | `make verify` (typecheck) — merge-blocking |
