...
| 4 | Spike QC used out-of-range neighbors in its median, **mislabeling valid readings next to a fault as spikes** | `qc._series_flags` | Take the median over QC-clean neighbors only |
...
| 8 | The a11y gate **double-counted** a redundantly-labeled control, masking a genuinely unlabeled one (failed open) | `scripts/a11y_check.py` | Evaluate labeling per control |
...
| 10 | `read(since/until)` compared timestamps **lexically**, so a valid non-canonical bound (offset, fractional seconds) silently dropped rows | `store.read` | Normalize bounds via `parse_timestamp`/`format_timestamp` |
...
datetime; `-0.0` coefficient could break byte-for-byte reproducibility (now normalized);
...
at capacity (now amortized O(1) with a cached count + batched trim); a firmware docstring
mismatch.
