...
| E3 | **Citable, versioned data snapshot + citation string / DOI** — a frozen, identified release a researcher/journalist can publish against. ✅ Implemented 2026-07-03 (PR #62) — `swelter snapshot --store <dir> --out <dir> [--version] [--doi]` freezes the raw observations, `corrections.yaml`, and `aggregate.geojson` into a versioned release with `MANIFEST.json` (per-file SHA-256, record count, observation time-window), a dataset `DATA-CITATION.cff` (CFF 1.2.0, `type: dataset`, CC0-1.0, distinct from the repo's software `CITATION.cff`), and a ready-to-paste `CITATION.txt`; local artifacts only, `--doi` is a fillable field with an honestly-labelled placeholder when omitted | E1-res, E2-journ, D4 | P1 | M | **[reuse][commons]** · **[extends ROADMAP 5.2 · NET-NEW]** |
...
| E9 | **Accessibility + bilingual compliance hook** for agency partners — package WCAG 2.2 AA + en/es against the DOJ ADA Title II rule. ✅ Implemented 2026-07-03 — `docs/AGENCY-COMPLIANCE-PACK.md`, all claims cited from existing repo evidence (ACR, ACCESSIBILITY-STANDARD, I18N, USER-RESEARCH) | C1, C2, F1 | P2 | S | **[language][a11y]** · **[corroborates ROADMAP 5.4]** |
