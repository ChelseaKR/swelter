...
prioritized against. High-stakes claims (mortality ranking, calibration validity,
...
- **[a11y]** Accessible data viz requires a screen-reader **data-table equivalent**
  for every non-trivial chart, never color alone, plus a plain-text summary.
...
| R4 | **Heat-index trustworthiness** — let a network co-locate/calibrate heat index, **or** derive it from already-calibrated temp + humidity, so heat stops being demo-provisional where the inputs are confirmed; keep it honestly provisional where they aren't. ✅ Implemented 2026-07-03 (`roadmap/r4-heat-index-trustworthiness-calibrate-`) — `calibrate.apply()` derives a calibrated `heat_index_c` (version `heat_index_c.derived-enclosure.{node_id}`) from a node's calibrated `temp_c` + co-timed humidity via `models.heat_index_c`, propagating the temp correction's `residual_std` scaled by the local `|∂HI/∂T|` (floored at 1.0) as uncertainty; stays raw/provisional wherever temp is raw or humidity is missing; adds no entry to `corrections.yaml` (still byte-for-byte reproducible) — see ADR 0014 and `calibration.md`'s "Heat index: derived, not fitted" | C2, A1, A2, E2 | P1 | M | **[heat-deadliest]** + `calibration.md` · **[NET-NEW vs ROADMAP]** |
| R5 | **Caveats travel with the value** — propagate the hourly-vs-24-h AQI window note and the provisional flag into every legend, marker, alert headline, and export, not just `api.md`. ⚙️ Partially implemented 2026-07-03 (PR #71) — data/API layer only (FIX-04): `aggregate.py`/`models.py` replaced the mean-of-sigmas cell `uncertainty` with a proper standard error (`sqrt(sum(sigma_i^2))/n`, documented within-cell-correlation caveat), published the old number separately as `mean_member_sigma` so no consumer silently reinterprets one as the other, gave the derived `exposure` cell an `uncertainty_note` naming which component (heat/air/both) bounds its ordinal level instead of a fabricated σ, and added an EPA NowCast PM2.5 variant (`aqi_window="nowcast"`) alongside the unchanged hourly-mean reading — closing F14's aggregation-layer gap. `docs/api.md` and `docs/VERSIONING.md` document all of it. The UI-propagation half (legend/marker/alert-headline rendering in `web/`) is still outstanding — out of scope here by repo policy (agents do not modify `web/`) | D2, C1, E2 | P1 | M | **[standards][trust]** · **[corroborates audit D + api.md]** |
...
| R7 | **Host-facing "what's public about your node" preview** — show a host exactly the grid-snapped coordinate the map/API publishes, so coarse-by-default is *visible*. ✅ Implemented 2026-07-03 — `swelter node-preview [node_id]` prints each placed node's private exact coordinate vs. its published (grid-snapped or precise) coordinate, the published cell id/label, the haversine offset in meters, and a prominent warning when `location: precise` disables coarse-by-default | B1 | P2 | S | privacy/DPIA · **[NET-NEW]** |
...
| ID | Item | Personas | Pri | Effort | Evidence · tag |
| --- | --- | --- | --- | --- | --- |
| E1 | **Plain-language neighborhood exposure brief / advocacy export** — "this block ran Danger N days this month," with sourced canopy/AC/redlining context, built on the existing export surface. ✅ Implemented 2026-07-10 (`roadmap/e1-exposure-brief-advocacy-export`, PR #93) — `swelter brief` builds per-cell historical Danger-day briefs via `exposure_brief.build_briefs`, with optional canopy/AC-access/redlining context layers; see ADR 0018 | C3, C1, R(esidents) | P1 | M | **[ej-distribution][compound][commons]** · **[corroborates ROADMAP 5.5]** |
...
| E3 | **Citable, versioned data snapshot + citation string / DOI** — a frozen, identified release a researcher/journalist can publish against. ✅ Implemented 2026-07-03 (PR #62), with source-rights hardening in the current release — `swelter snapshot --store <dir> --out <dir> [--version] [--doi]` freezes the raw observations, `corrections.yaml`, and `aggregate.geojson` into a versioned release with `MANIFEST.json` (per-file SHA-256, record count, observation time-window), a source-specific `DATA-LICENSE`, a dataset `DATA-CITATION.cff` (CFF 1.2.0, `type: dataset`, distinct from the repo's software `CITATION.cff`), and a ready-to-paste `CITATION.txt`; provider-specific OpenAQ snapshots also require the validated per-location `source-license-ledger.json`; local artifacts only, `--doi` is a fillable field with an honestly-labeled placeholder when omitted | E1-res, E2-journ, D4 | P1 | M | **[reuse][commons]** · **[extends ROADMAP 5.2 · NET-NEW]** |
| E4 | **"No local reference" / transfer-calibration path** — guidance + tooling to calibrate when no regulatory monitor is nearby (traveling reference unit; chained co-location to a distant AQS/AirNow site), provenance recorded honestly | B3, C1 | P1 | L | **[monitoring-gap][calibration][drift]** · **[NET-NEW]** |
| E5 | **Privacy-preserving alert delivery recipe** — a documented, collective-run SMS/WhatsApp bridge where contact details **never touch swelter**, hardening the existing webhook pattern. ✅ Implemented 2026-07-02 — `docs/alerts.md` "As a webhook (collective-run)" section split into a generic chat-webhook recipe and a hardened SMS/WhatsApp recipe: dedup via a committed seen-alert-ids state file (`actions/cache` alternative noted with pinned SHA), `area_id` allowlist so residents only see their block, a `MAX_PER_RUN` cost cap, a provider-agnostic gateway call (no vendor/key baked in), and an explicit note that templating/opt-in/carrier compliance is the collective's responsibility; ADR 0010 gets a one-line pointer update | A4, C3 | P2 | M | **[ej-distribution]** + ADR 0010 · **[extends alerts.md · NET-NEW]** |
| E6 | **Round-trip interoperability proof** — swelter → SensorThings → a standard client, and a parameter crosswalk to OpenAQ/Sensor.Community, so swelter feeds the commons it draws from. ✅ Implemented 2026-07-02 — `src/swelter/crosswalk.py` (outbound crosswalk, the inverse of the inbound OpenAQ/Sensor.Community maps), `tests/test_crosswalk.py` + `tests/test_roundtrip_interop.py` (a plain-dict "standard client" walks `swelter.api`'s SensorThings JSON, recovers values/units, then resolves every parameter to its commons label), `swelter crosswalk` CLI, `docs/interop-crosswalk.md` | D4, E3-dev, E1-res | P2 | M | **[commons][reuse]** · **[corroborates ROADMAP 5.3 + ADR 0007]** |
...
| E9 | **Accessibility + bilingual evidence hook** for agency partners — package the WCAG 2.2 AA target and en/es evidence against the DOJ ADA Title II rule without presenting automation as certification. ✅ Implemented 2026-07-03 — `docs/AGENCY-COMPLIANCE-PACK.md`, with current human NVDA/VoiceOver and independent Spanish signoff explicitly tracked in [issue #106](https://github.com/ChelseaKR/swelter/issues/106); claims are tied to the ACR, accessibility standard, I18N evidence, and user research | C1, C2, F1 | P2 | S | **[language][a11y]** · **[corroborates ROADMAP 5.4]** |
...
| E11 | **Caveat-baked share card / screenshot** — an export image that carries provisional + hourly-window labels so a number can't travel context-free. ✅ Implemented 2026-07-02 — `web/app.js` `buildShareCanvas`/`saveShareCard` render a canvas PNG with the reading, measurement + hour, the hourly-window caveat, and (when provisional) a baked-in provisional band; "Save share card" button in the detail panel, en/es via `t()`; see ADR 0016 | E2-journ, C2 | P2 | S | **[trust][standards]** · **[NET-NEW]** |
...
substitute for it.** Do not prioritize off this document alone. Use it to design the
