...
// heat island legible at a glance, and provisional cells never get a color.
const HEAT_SCALE = [
  [36, "heat-5"],
  [32, "heat-4"],
  [28, "heat-3"],
...
async function loadStrings(lang) {
  ...
  try {
    ...
    // Keep the last complete catalog and its matching document language. A failed language swap
...
function localizeDocumentMetadata() {
  // Never feed `t()`'s raw-key fallback into browser chrome. A failed catalog request should
...
function renderDemoContract() {
  ...
  // override and keep the catalog's sensor wording.
...
// The legend adapts to the active measurement so the marker colors are interpretable: PM2.5 shows
...
// the level NAME in text (and a swatch/pattern), never by color alone.
const LEGEND_BLOCK = {
  pm25_ugm3: "pm25",
  temp_c: "temp",
  heat_index_c: "temp",
...
// and severity is never carried by color alone, so scaling the root font and swapping the color
...
async function init() {
  ...
  // chain of sequential round-trips: the demo contract, string catalog, and basemap have no
