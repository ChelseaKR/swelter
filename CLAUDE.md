...
- **`config.py`** — `NetworkConfig`, `NodeConfig` (`location: coarse|precise`), `ReferenceMonitor`,
  `CalibrationWindow`, `snap_to_grid(lat, lon, grid_m)` → grid-cell centre, `load_config(network.yaml)`,
...
- **`store.py`** — the `Store` protocol plus `SqliteStore` (default). The store is a copyable folder:
  ...
  `drop_calibrated()` drops the derived (non-raw) records so they can be rebuilt from raw alone
  (the rebuild itself is `swelter rebuild` / `calibrate.apply`). Pluggable (a Parquet backend could
