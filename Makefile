...
# No rule is excluded from this scan. Five used to be, repository-wide, on arguments that are true
# of the seven lines they were written for and are not true of a file nobody has written yet: a
# blanket `--exclude-rule` exempts the next raw query, the next `HTTPSConnection`, and the next
# `urlopen` as silently as it exempts these. Each of those seven lines now carries its own
# `# nosemgrep:` with its own reason, so a new site fails this gate instead of inheriting an
# exemption. Measured 2026-09-08 with the pinned Semgrep over 349 tracked files and 510 rules:
# strip every inline comment and this scan reports 9 findings on those 7 lines (two lines match
# two rules each); put them back and it reports 0. The re-narrowing #107 asked for; the count it
# costs is in scripts/hygiene_check.py.
